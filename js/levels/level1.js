// let  level1;
// function initLevel()

import { Background } from "../../models/background.class.js";
import { Chicken } from "../../models/chicken.class.js";
import { Clouds } from "../../models/clouds.class.js";
import { Collectable_bottols } from "../../models/collectable_Bottle.class.js";
import { Collectable_coins } from "../../models/collectable_coins.class.js";
import { Endboss } from "../../models/endboss.class.js";
import { Level } from "../../models/level.class.js";
import { MiniChicken } from "../../models/MiniChicken.class.js";

// {
export const level1 = new Level(
    [
        new MiniChicken(),
        new Chicken(),
        new Chicken(),
        new Chicken(),
        new MiniChicken(),
        new Chicken(),
        new Chicken(),
        new Chicken(),
        new MiniChicken(),
        new Chicken(),
        new Chicken(),
        new MiniChicken(),
        new Endboss(),
    ],
    [
        new Clouds(),
        new Clouds(),
        new Clouds(),
        new Clouds(),
        new Clouds(),
        new Clouds(),
    ],
    [
        new Background("img/5_background/layers/air.png"),
        new Background("img/5_background/layers/3_third_layer/2.png"),
        new Background("img/5_background/layers/2_second_layer/2.png"),
        new Background("img/5_background/layers/1_first_layer/2.png"),

        new Background("img/5_background/layers/air.png"),
        new Background("img/5_background/layers/3_third_layer/1.png"),
        new Background("img/5_background/layers/2_second_layer/1.png"),
        new Background("img/5_background/layers/1_first_layer/1.png"),
        new Background("img/5_background/layers/air.png"),
        new Background("img/5_background/layers/3_third_layer/2.png"),
        new Background("img/5_background/layers/2_second_layer/2.png"),
        new Background("img/5_background/layers/1_first_layer/2.png"),

        new Background("img/5_background/layers/air.png"),
        new Background("img/5_background/layers/3_third_layer/1.png"),
        new Background("img/5_background/layers/2_second_layer/1.png"),
        new Background("img/5_background/layers/1_first_layer/1.png"),
        new Background("img/5_background/layers/air.png"),
        new Background("img/5_background/layers/3_third_layer/2.png"),
        new Background("img/5_background/layers/2_second_layer/2.png"),
        new Background("img/5_background/layers/1_first_layer/2.png"),

        new Background("img/5_background/layers/air.png"),
        new Background("img/5_background/layers/3_third_layer/1.png"),
        new Background("img/5_background/layers/2_second_layer/1.png"),
        new Background("img/5_background/layers/1_first_layer/1.png"),
        new Background("img/5_background/layers/air.png"),
        new Background("img/5_background/layers/3_third_layer/2.png"),
        new Background("img/5_background/layers/2_second_layer/2.png"),
        new Background("img/5_background/layers/1_first_layer/2.png"),

        new Background("img/5_background/layers/air.png"),
        new Background("img/5_background/layers/3_third_layer/1.png"),
        new Background("img/5_background/layers/2_second_layer/1.png"),
        new Background("img/5_background/layers/1_first_layer/1.png"),
        new Background("img/5_background/layers/air.png"),
        new Background("img/5_background/layers/3_third_layer/2.png"),
        new Background("img/5_background/layers/2_second_layer/2.png"),
        new Background("img/5_background/layers/1_first_layer/2.png"),
    ],
    [
        // --- 30 Collectable Coins Coordinates (Max X: 2500) ---
        new Collectable_coins(-200, 200),
        new Collectable_coins(-100, 170),
        new Collectable_coins(0, 200),
        new Collectable_coins(100, 160),
        new Collectable_coins(200, 180),
        new Collectable_coins(300, 200),
        new Collectable_coins(400, 150),
        new Collectable_coins(500, 180),
        new Collectable_coins(600, 200),
        new Collectable_coins(700, 160),
        new Collectable_coins(800, 190),
        new Collectable_coins(900, 170),
        new Collectable_coins(1000, 200),
        new Collectable_coins(1100, 150),
        new Collectable_coins(1200, 180),
        new Collectable_coins(1300, 200),
        new Collectable_coins(1400, 160),
        new Collectable_coins(1500, 190),
        new Collectable_coins(1600, 170),
        new Collectable_coins(1700, 200),
        new Collectable_coins(1800, 150),
        new Collectable_coins(1900, 180),
        new Collectable_coins(2000, 200),
        new Collectable_coins(2100, 160),
        new Collectable_coins(2200, 190),
        new Collectable_coins(2300, 170),
        new Collectable_coins(2400, 200),
        new Collectable_coins(2500, 150),
        new Collectable_coins(2600, 180),
        new Collectable_coins(2700, 200),
        new Collectable_coins(2800, 160),
        new Collectable_coins(2900, 190),
        new Collectable_coins(3000, 170),
    ],
    [
        new Collectable_bottols(-150, 350),
        new Collectable_bottols(-60, 350),

        new Collectable_bottols(120, 350),
        new Collectable_bottols(210, 350),
        new Collectable_bottols(300, 350),

        new Collectable_bottols(480, 350),
        new Collectable_bottols(570, 350),

        new Collectable_bottols(750, 350),
        new Collectable_bottols(840, 350),

        new Collectable_bottols(1020, 350),
        new Collectable_bottols(1110, 350),

        new Collectable_bottols(1290, 350),
        new Collectable_bottols(1380, 350),

        new Collectable_bottols(1560, 350),
        new Collectable_bottols(1650, 350),

        new Collectable_bottols(1830, 350),
        new Collectable_bottols(1920, 350),

        new Collectable_bottols(2100, 350),
        new Collectable_bottols(2190, 350),

        new Collectable_bottols(2390, 350),
        new Collectable_bottols(2500, 350),

        new Collectable_bottols(2680, 350),
        new Collectable_bottols(2770, 350),

        new Collectable_bottols(2950, 350),
        new Collectable_bottols(3040, 350),
    ],
);
// }
